...
const separar_palabras = (oracion:string): string[] => {
    const palabras = oracion.match(/[\w]+|[.,!?;]/g);
    return palabras ? palabras: [];
};

function imprimir() {
    const textarea_heyluc = document.getElementById("heyluc-text") as HTMLTextAreaElement;
    const textarea_espanol = document.getElementById("espanol-text") as HTMLTextAreaElement;
    const oracion = textarea_heyluc.value;
    const palabras = separar_palabras(oracion);
    let traducciones: string[] = [];

    palabras.forEach((palabra) => {
        const traduccion:string = diccionario[palabra.toLowerCase()];
        if (traduccion) {
...
const button_traducir = document.getElementById("button_traducir") as HTMLButtonElement;
